...
class Wall:
   def __init__(self):
      ...
      #list of wall block positions
      self.placement = [(100, 100),
                        (110, 110),
                        (120, 120),
                        (130, 130)
      ]
   def draw(self, surface):
      for segment in self.placement:
         pygame.draw.rect(
            surface,
            (158, 255, 0),
   ...
   def check_collision(self, snake_head):
      return snake_head in self.placement
class Snake:
   """
...
def game_loop():
   ...
   #music
   pygame.mixer.music.load(r'C:\\Users\\Robert\\Downloads\\ton-ton-ton FUNK.mp3')
   ...
   while True:
       ...
       if random.randint(1, 2) == 1:
         food.direction = random.choice(['UP', 'DOWN', 'LEFT', 'RIGHT'])
       if food_timer >= 2:
         food.move(width, height)  # Move food to new position
         food_timer = 0
